...
- **Very large dumps at bounded memory.** It streams the dump in two passes and keeps
  peak RSS well below the heap size. A production dump with a **20 GB live Java heap** —
...
confidential); only the resource numbers are reported here. The dump holds a **20 GB
