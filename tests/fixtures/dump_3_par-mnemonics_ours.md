...
- **Shape:** deep — long dominator chains suggest nested collections or linked structures; trace the chain to find the retaining root — 90% of objects within depth 9, max depth 28. See [Dominator-Depth Distribution](#dominator-depth-distribution).
...
- **Thread Pinning:** thread `ForkJoinPool.commonPool-worker-16` retains 8.9 MB (46.3% of heap) and pins 316 thread-local roots — a live thread is holding a disproportionate amount of memory alive. Inspect the thread's stack frames and ThreadLocal values in the Threads section. See [Threads](#threads).
...
- **Empty-Collection Cemetery:** 2,950 of 3,982 tracked collections (74.1%) are empty — pre-allocated but never populated containers waste object-header overhead. Consider lazy initialization, returning `Collections.emptyList()` sentinels, or using `null` until the collection is first written. See [Collections](#collections).
- **Collection Waste Not Analyzed:** _Collection waste not analyzed — re-run with `--collections` to check for wasted capacity._

## Waste Summary

_Approximately **2.3 MB** estimated reclaimable across the sources below — duplicate strings, duplicate primitive arrays, boxed primitives, and empty/singleton collection overhead. Fix the biggest category first for the highest impact. Figures are approximate; sources may overlap._
...
_… 2,394 more classes, 388.8 KB shallow / 1.5 MB retained (full list in JSON)._
...
- `java.util.HashMap$Node[]` (shallow 512.0 KB, retained 8.4 MB)
  ...
  - `java.util.HashMap$Node` ×21 (shallow 32 B, retained 504 B each)
    ...
    - `java.lang.String` (shallow 24 B, retained 96 B)
      - `byte[]` (shallow 72 B, retained 72 B)
  _… (272 more siblings — full data in JSON)_
...
_Objects retaining far more than their largest single child — memory held directly in the object or spread across many small dominated children. Drop = object retained − largest child retained (memory reclaimed if this object became unreachable, net of what the biggest child already accounts for). Threshold 0.2 MB (1% of reachable heap). Multiple rows with the same class are distinct objects._
...
_520 tracked of 3,372 maps (occupied slots ÷ capacity; high values ≥ 90% increase collision chains)._
