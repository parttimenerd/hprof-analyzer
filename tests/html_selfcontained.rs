...
fn bundle_is_within_size_budget() {
    // The raw minified React + Chart.js + d3-dag bundle ships in the binary and
    // in every HTML file. Budget: <= 856 KB minified (raised from 852 KB to
    // accommodate forward nav, peer instances panel, cycle badge). Keep in sync with
    // web/esbuild.config.mjs budgetBytes.
