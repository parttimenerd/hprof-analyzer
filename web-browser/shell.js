  ...
  function handleTab() {
    ...
    if (line.startsWith('/') && !line.includes(' ')) {
      ...
      const cmds = ['help','clear','status','analyze','history','export','set','classes','plan','explain','filter','grep',
                    'sort','unique','stats','top','head','tail','cols','columns','select','rename','wc','obj','run','bookmark','save','forget','last','describe','count','watch','q','quit','disconnect'];
    ...
    if (cmd === '/export' || cmd.startsWith('/export ')) {
      ...
      if (fmt === 'csv') {
        ...
        const objs = lastResult.rows.map(row => {
          const obj = {};
          lastResult.columns.forEach((col, i) => { obj[col] = fmtCell(row[i], col); });
          return obj;
        });
