  ...
  async function handleEnter(text) {
    ...
    if (cmd.startsWith('/unique ') || cmd === '/unique') {
      ...
      if (shown < totalDistinct) {
        term.writeln(`\x1b[2m${shown} of ${totalDistinct} distinct values shown (top ${showN})\x1b[0m`);
      } else {
    ...
    if (cmd === '/distinct' || cmd === '/dedup') {
      if (!lastResult) {
        ...
        const note = `${kept.length} unique rows (${removed} duplicate${removed !== 1 ? 's' : ''} removed)`;
    ...
    if (cmd === '/fields' || cmd.startsWith('/fields ')) {
      ...
      if (matches.length === 0) {
        ...
        const maxLen = matches.reduce((m, f) => Math.max(m, f.length), 0);
        const colW = maxLen + 2;
        const cols = Math.max(1, Math.floor((term.cols - 4) / colW));
        for (let i = 0; i < matches.length; i += cols) {
          const row = matches.slice(i, i + cols);
          term.writeln('  ' + row.map(f => f.padEnd(colW)).join('').trimEnd());
        }
    ...
    if (cmd === '/set' || cmd.startsWith('/set ')) {
      ...
      if (!args[0]) {
        ...
        term.writeln(`  \x1b[1mlimit\x1b[0m    \x1b[32m${settings.rowLimit === Infinity ? 'unlimited' : settings.rowLimit}\x1b[0m  \x1b[2m(rows displayed per query; 0 or "unlimited" = no cap)\x1b[0m`);
        term.writeln(`  \x1b[1mbytes\x1b[0m    \x1b[32m${settings.bytesRaw ? 'raw' : 'human'}\x1b[0m  \x1b[2m(raw = show bytes as numbers)\x1b[0m`);
        term.writeln(`  \x1b[1mnull\x1b[0m     \x1b[32m"${settings.nullStr}"\x1b[0m  \x1b[2m(how null values display)\x1b[0m`);
        term.writeln(`  \x1b[1mcolor\x1b[0m    \x1b[32m${settings.color ? 'on' : 'off'}\x1b[0m  \x1b[2m(colorize table cells)\x1b[0m`);
