  ...
  function handleTab() {
    ...
    if (line.startsWith('/') && !line.includes(' ')) {
      ...
      const cmds = ['help','clear','status','analyze','history','export','set','classes','filter',
                    'sort','unique','top','run','bookmark','save','forget','last','describe','count','watch','q','quit','disconnect'];
    ...
    if (line.startsWith('/set ')) {
      const partial = line.slice(5).trim().toLowerCase();
    ...
    if (line.startsWith('/sort ') || line.startsWith('/filter ') || line.startsWith('/unique ')) {
      if (lastResult && lastResult.columns.length > 0) {
        const pfxLen = line.startsWith('/sort ') ? 6 : line.startsWith('/unique ') ? 8 : 8;
        const partial = line.slice(pfxLen).toLowerCase();
