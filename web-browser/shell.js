    ...
    if (cmd.startsWith('/sample ') || cmd === '/sample') {
      if (!lastResult) {
        ...
        if (isNaN(n) || n <= 0) {
          ...
          const sampledResult = { columns: lastResult.columns, rows: sampled, row_count: sampled.length };
      ...
      if (!data.ok) {
        ...
        if (r.error) {
          ...
          if (rows.length > 20) {
            ...
            term.writeln(`\x1b[2m  /filter <text|/re/>  /sort [-]<col>  /select <col>…  /pivot <col>  /row [N]${statHint}  /export [csv]\x1b[0m`);
          }
        } else {
