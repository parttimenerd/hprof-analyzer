...
function startTerminal() {
  ...
  async function handleEnter(text) {
    ...
    if (cmd.startsWith('/unique ') || cmd === '/unique') {
      ...
      const barCap = Math.max(0, term.cols - colW - cntW - 6);
      const showBar = barCap >= 8;
      const hdr = `${colName.padEnd(colW)}  ${'count'.padStart(cntW)}${showBar ? '  bar' : ''}`;
      ...
      entries.slice(0, settings.rowLimit).forEach(([val, cnt]) => {
        ...
        term.writeln(`${val.padEnd(colW)}  \x1b[32m${String(cnt).padStart(cntW)}\x1b[0m${bar}`);
      });
