...
function startTerminal() {
  ...
  function handleTab() {
    ...
    if (lastResult && lastResult.columns.length > 0) {
      // /sort supports comma-separated multi-column: complete the last segment
      if (line.startsWith('/sort ')) {
        ...
        const partial = (lastComma >= 0 ? afterCmd.slice(lastComma + 1) : afterCmd).trim().toLowerCase();
        const cols = lastResult.columns.filter(c => c.toLowerCase().startsWith(partial));
        if (cols.length === 1) { setLine(prefix + cols[0]); }
      ...
      if (matched) {
        const partial = line.slice(matched.length).toLowerCase();
        const cols = lastResult.columns.filter(c => c.toLowerCase().startsWith(partial));
        if (cols.length === 1) { setLine(matched + cols[0]); }
        else if (cols.length > 1 && cols.length <= 20) {
          term.writeln('');
          term.writeln('  ' + cols.map(c => `\x1b[36m${c}\x1b[0m`).join('  '));
          redrawLine();
      ...
      if (!lastResult) {
        ...
        fields.forEach((f, i) => {
          ...
          for (const row of lastResult.rows) {
            ...
            if (typeof cell !== 'object') { typeTag = typeof cell; break; }
            if (cell.kind === 'null') continue;
            typeTag = cell.kind || typeof cell; break;
          }
          term.writeln(`  \x1b[2m${String(i + 1).padStart(idxW)}\x1b[0m  \x1b[36m${f.padEnd(colW)}\x1b[0m  \x1b[2m${typeTag}\x1b[0m`);
        });
