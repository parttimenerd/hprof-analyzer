    ...
    try {
      ...
      if (!data.ok) {
        ...
        if (r.error) {
          ...
          lastResult = { columns: colNames, rows, note: r.note, truncated: r.truncated, row_count: r.row_count };
          currentRowIdx = 0;
          const trunc = r.truncated ? `  \x1b[33m[capped at ${r.row_count} rows — add LIMIT N or increase with LIMIT 0 for all]\x1b[0m` : '';
          const elapsedFmt = elapsedMs < 1 ? `${(elapsedMs * 1000).toFixed(0)}µs` : elapsedMs < 1000 ? `${elapsedMs.toFixed(1)}ms` : `${(elapsedMs / 1000).toFixed(2)}s`;
          ...
          term.writeln(`\x1b[2m${elapsedMs < 1 ? (elapsedMs * 1000).toFixed(0) + 'µs' : elapsedMs < 1000 ? elapsedMs.toFixed(1) + 'ms' : (elapsedMs / 1000).toFixed(2) + 's'}\x1b[0m`);
        }
      }
    } catch (e) {
      clearInterval(spinTimer);
