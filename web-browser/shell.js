...
function renderWasmReport(report, fileName) {
  ...
  if (threads.length > 0) {
    ...
    tHtml += `</tbody></table>`;
    parts.push(sec('threads', 'Threads', `${threadsFiltered.length} thread(s).`, tHtml));
  }

  // Build nav bar from registered sections
  const navHtml = navLinks.length > 0
    ? `<nav class="rpt-nav">${navLinks.map(({ id, title }) =>
        `<a class="rpt-nav-link" href="#${id}">${escHtml(title)}</a>`).join('')}</nav>`
    : '';
