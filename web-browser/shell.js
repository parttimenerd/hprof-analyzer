...
function startTerminal() {
  ...
  async function handleEnter(text) {
    ...
    if (cmd === '/fields' || cmd.startsWith('/fields ')) {
      ...
      if (matches.length === 0) {
        term.writeln(pattern
          ? `\x1b[33mNo fields matching "${pattern}"\x1b[0m`
  ...
  function printHelp() {
    ...
    c('/plan <oql>',             '— show query execution plan without scanning (/explain is an alias)');
    c('/classes [pat]',          '— list class names filtered by pattern');
    c('/fields [pat]',           '— list instance field names filtered by pattern');
    c('/describe <cls>',         '— show fields of a class');
    c('/count [cls|oql]',        '— count instances (class name) or rows (oql); no arg = last result shape');
    c('/obj <cls>#<idx>',        '— inspect a specific object by class + dense index');
    h('Result post-processing');
    c('/last',                   '— re-display last result');
    c('/wc [col]',               '— shape (rows × cols); with col: count non-null values');
