...
**zstd-compressed header spool** (`mat/int_index_1n.rs`): The IntArray1N sorted format requires the header (one i32 body-position per object, ~2 GB raw) to be written *after* the body. We cannot hold a plain `Vec<i32>` without blowing the budget. Instead we stream header values through a zstd level-3 encoder (`Encoder<Vec<u8>>`) as the body is written — but with **delta encoding**: filled entries store `(delta_from_prev_filled + 1)` and holes store `0`, so the sequence is small integers (mostly 2–10) rather than large monotone values. Delta-encoded small integers compress to ~1 byte each; the 2 GB raw header shrinks to ~100 MB. After the body is done, `fwd_tgt` and `class_obj_ids` are dropped, and the blob is decompressed streaming through a 64 KiB buffer — reconstructing original positions via running prefix sum — into the header `IntIndexStreamer`.
