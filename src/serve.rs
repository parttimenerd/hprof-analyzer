...
pub fn run_server(path: &str, port: u16, opts: AnalyzeOptions) -> io::Result<()> {
    ...
    println!();
    println!("Analysis:");
    println!("  GET  /status                    → not_started | analyzing | ready | failed");
    println!("  POST /analyze                   → trigger full analysis (run first!)");
    println!();
    println!("Report sections (add ?format=md for Markdown):");
    println!("  GET  /report                    → full report JSON");
    ...
    println!("  POST /query    -d 'SELECT …'    → alias of POST /");
    println!("  POST /stream   -d 'SELECT …'    → NDJSON rows");
    println!("  GET  /help                      → OQL language reference");
    println!("  GET  /schema                    → QueryResult JSON schema");
    println!("  GET  /version                   → endpoint catalog");
    println!();
    println!("After POST /analyze completes, OQL queries can use @retainedHeapSize,");
    println!("dominators(), and @inbounds.");
    println!();
    println!("examples:");
    println!("  curl -s http://{bound}/status");
    println!("  curl -s -X POST http://{bound}/analyze");
    println!("  curl -s 'http://{bound}/report?format=md'");
    println!("  curl -s http://{bound}/ -d 'SELECT @displayName FROM java.lang.Thread'");
    println!("  curl -s http://{bound}/ -d 'SELECT @retainedHeapSize FROM java.lang.Thread'");
