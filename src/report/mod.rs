...
mod tests {
    ...
    fn test_accumulation_point_big_drop_and_leaf() {
        ...
        let total_sh: u64 = (0..g.n)
            .filter(|&i| g.idom[i] != u32::MAX)
            .map(|i| g.shallow[i] as u64)
            .sum();
        let l = build_leak_suspects(&g, &dc_off, &dc_tgt, DOMINATED_CAP, 30, 5000, 20, total_sh);
    ...
    fn test_accumulation_dominated_cap_truncates() {
        ...
        let total_sh: u64 = (0..g.n)
            ...
            .map(|i| g.shallow[i] as u64)
            .sum();
        let l1 = build_leak_suspects(&g, &dc_off, &dc_tgt, 1, 30, 5000, 20, total_sh);
        ...
        let l2 = build_leak_suspects(&g, &dc_off, &dc_tgt, DOMINATED_CAP, 30, 5000, 20, total_sh);
